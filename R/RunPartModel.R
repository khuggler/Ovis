...
#' @param gpsdat data.frame of gps data to run model that includes the following columns (in this order): 'IdCol', 'TelemDate', 'x', 'y'. Optional columns include: 'dop', 'X2D.3D', but recommended if available to help improve data cleaning process. IdCol = column that represents unique identifier, TelemDate = as.POSIXct format. Fix time and date, x = x coordinate, y = y coordinate. See data(sheep.gps) for example of expected format.
...
#' @return Resulting object is a pdf with movement metrics for each unique animal 
